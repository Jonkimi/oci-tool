...
class Cmd:
    ...
    def __exec_cmd(self):
        ...
        PROXY = get_proxy()
        if PROXY:
            complete_process = subprocess.run(self.cmd, capture_output=True, shell=True, env={"https_proxy": PROXY})
...
class OciPublicIpDeleteCmd(Cmd):
    def __init__(self, public_ip_id):
        self.cmd = f"echo y | oci network public-ip delete --public-ip-id {public_ip_id}"
...
class OciPublicIpCreateCmd(Cmd):
    def __init__(self, compartment_id, private_ip_id):
        self.cmd = (
            f"oci network public-ip create -c {compartment_id} --private-ip-id {private_ip_id} --lifetime EPHEMERAL"
        )
